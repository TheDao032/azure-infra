locals {
  ...
  jumpbox_vm_passwd = get_env("JUMPBOX_PASSWD")
}
...
dependency "secrets" {
  ...
  mock_outputs = {
    secrets = {
      jumpboxAgentUsername = "jumpbox"
      jumpboxAgentPassword = "Jumpbox@1234"
    }
  }
...
dependencies {
  paths = ["../resource_group", "../log_analytics", "../virtual_network", "../secrets"]
}
...
inputs = {
  ...
  jumpbox_vm_username = dependency.secrets.outputs.secrets["jumpboxAgentUsername"]
  jumpbox_vm_passwd   = dependency.secrets.outputs.secrets["jumpboxAgentPassword"]
