...
LOG_FILE_NAME=${1:-plan-${ENVIRONMENT}.log}

# Run plan all and display output both to terminal and the log file temp.log
az login --service-principal --username ${ARM_CLIENT_ID} --password ${ARM_CLIENT_SECRET} --tenant ${ARM_TENANT_ID}
az storage blob upload-batch --account-name ${STORAGE_ACCOUNT_NAME} --destination terraform-state --source ./scripts --pattern configure_self_host_agent.sh --overwrite

cd terraform/environments/subscription-${ENVIRONMENT}/${LOCATION}/${ENVIRONMENT}

terragrunt run-all validate -no-color --terragrunt-non-interactive --terragrunt-include-external-dependencies 2>&1 | tee temp.log
terragrunt run-all plan -no-color --terragrunt-non-interactive --terragrunt-include-external-dependencies 2>&1 | tee temp.log
